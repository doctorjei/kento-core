...
def test_start_lxc(mock_root, mock_run, mock_running, tmp_path):
    d = tmp_path / "mybox"
    d.mkdir()
...
@patch("kento.subprocess_util.subprocess.run", side_effect=_ok)
@patch("kento.start.require_root")
def test_start_pve(mock_root, mock_run, mock_running, tmp_path):
    d = tmp_path / "100"
    d.mkdir()
    (d / "kento-image").write_text("debian:12\n")
...
@patch("kento.subprocess_util.subprocess.run", side_effect=_ok)
@patch("kento.start.require_root")
def test_start_defaults_to_lxc(mock_root, mock_run, mock_running, tmp_path):
    d = tmp_path / "mybox"
    d.mkdir()
    (d / "kento-image").write_text("debian:12\n")
    # No kento-mode file
...
@patch("kento.start.is_running", return_value=False)
@patch("kento.start.require_root")
def test_start_vm(mock_root, mock_running, tmp_path):
    d = tmp_path / "testvm"
    d.mkdir()
    (d / "kento-image").write_text("debian:12\n")
...
class TestStartPveVm:
    ...
    @patch("kento.subprocess_util.subprocess.run", side_effect=_ok)
    @patch("kento.start.require_root")
    def test_start_calls_qm(self, mock_root, mock_run, mock_running, tmp_path):
        d = tmp_path / "test"
        d.mkdir()
